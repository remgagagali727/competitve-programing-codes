...
struct SegmentTree{
    ...
    stNode unir(stNode n1, stNode n2) {
        stNode toReturn;
        if(!n1.cuenta) return n2;
        if(!n2.cuenta) return n1;
        toReturn.cuenta = true;
        toReturn.derecha = n2.derecha;
        toReturn.izquierda = n1.izquierda;
        toReturn.suma = n2.suma + n1.suma + abs(n2.izquierda.first - n1.derecha.first)
            + abs(n2.izquierda.second - n1.derecha.second);
        return toReturn;
    }

    stNode inicializar(TV v) {
        stNode toReturn;
        toReturn.cuenta = true;
        toReturn.suma = 0;
        toReturn.derecha = toReturn.izquierda = v;
        return toReturn;
    }

    void initialize(vector<TV>& values) {
        initialize(0, n - 1, 0, values);
    }

    void initialize(int l, int r, int nodo, vector<TV>& values) {
        if(l == r) {
            t[nodo] = inicializar(values[l]);
        } else {
            int m = (l + r) / 2;
    ...
    void update(int posicion, TV valor) {
        update(0, n - 1, 0, posicion, valor);
    }

    void update(int l, int r, int nodo, int posicion, TV valor) {
        if(l == r) {
            t[nodo] = inicializar(valor);
        } else {
            int m = (l + r) / 2;
            if(posicion <= m) {
    ...
    stNode qry(int l, int r, int lt, int rt, int nodo) {
        if(r < lt || l > rt) return neutro;
        if(rt <= r && lt >= l) return t[nodo];
        int mt = (lt + rt) / 2;
        auto izquierda = qry(l, r, lt, mt, 2 * nodo + 1);
        auto derecha = qry(l, r, mt + 1, rt, nodo * 2 + 2);
        auto res = unir(izquierda, derecha);
        return res;
    }
};
...
void redirigir() {
    ifstream fileIn("marathon.in");
    cin.rdbuf(fileIn.rdbuf());
    ofstream fileOut("marathon.out");
