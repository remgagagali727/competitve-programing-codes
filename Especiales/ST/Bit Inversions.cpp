...
struct stNode{
    bool full;
    int maximo;
    pair<int, bool> derecha;
    pair<int, bool> izquierda;
};
...
struct SegmentTree{
    ...
    SegmentTree(int _n) {
        neutro.full = 1;
        neutro.maximo = 0;
        neutro.derecha = neutro.izquierda = {-1, 0};
        n = _n;
        t.resize(4 * n);
    }
    ...
    stNode unir(stNode n1, stNode n2) {
        stNode toReturn;
        if(n1.full && (n1.izquierda.second == n2.izquierda.second)) toReturn.izquierda = {n1.izquierda.first + n2.izquierda.first, n1.izquierda.second};
        else toReturn.izquierda = n1.izquierda;
        if(n2.full && (n2.derecha.second == n1.derecha.second)) toReturn.derecha = {n2.derecha.first + n1.derecha.first, n2.derecha.second};
        else toReturn.derecha = n2.derecha;
        toReturn.full = n1.full && n2.full && (n1.derecha.second == n2.izquierda.second || n1.derecha.first == -1 || n2.derecha.first == -1);
        toReturn.maximo = max(n1.maximo, n2.maximo);
        if(n1.derecha.second == n2.izquierda.second) toReturn.maximo = max(toReturn.maximo, n1.derecha.first + n2.izquierda.first);
        return toReturn;
    }
    ...
    stNode inicializar(bool v) {
        stNode toReturn;
        toReturn.maximo = 1;
        toReturn.derecha = toReturn.izquierda = {1, v};
        toReturn.full = true;
        return toReturn;
    }
    ...
    void update(int l, int r, int nodo, int posicion) {
        if(l == r) {
            t[nodo] = inicializar(!t[nodo].derecha.second);
        } else {
            int m = (l + r) / 2;
            if(posicion <= m) {
