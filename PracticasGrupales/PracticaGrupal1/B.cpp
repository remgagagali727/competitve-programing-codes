...
void sol() {
    ...
    nB /= 2;
    nG /= 2;
    vector<pair<ld,ld>> boys(nB);
    vector<pair<ld,ld>> girls(nG);
    int ig, ib;
    ig = ib = 0;
    ld ans = 0;
    for(int i = 0;i < n;i++) {
        ...
        if(s[i] == 'B') {
            if(ib < nB) {
                ...
                ans += sqrt(pow(boys[ib % nB].first - x, 2) + pow(boys[ib % nB].second - y, 2));
            }
            ib++;
        } else {
            if(ig < nG) {
                ...
                ans += sqrt(pow(girls[ig % nG].first - x, 2) + pow(girls[ig % nG].second - y, 2));
            }
            ig++;
        }
    }
