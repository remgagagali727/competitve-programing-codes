    ...
    for(int i = 0;i < n;i++) {
        if(lasty == points[i].first) {
            ...
            while(l <= r) {
                ...
                // }
                if(center == 100000) {
                    center = (a.second + b.second) / 2;
                    // cout << center << nl;
                }  else {
                    ld tcenter = (a.second + b.second) / 2;
                    // cout << tcenter << " " << center;
                    if(tcenter != center) {
                        posible = false;
                        break;
            ...
            helper = vector<pair<double, double>>();
            helper.pb(points[i]);
        }
        lasty = points[i].first;
    }
    int t = helper.size();
    int l = 0, r = t - 1;
    while(l <= r) {
