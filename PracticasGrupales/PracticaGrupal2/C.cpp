...
// void sol(int t) {
    ...
    while(cin >> n >> m >> sn) {
        ...
        for(int i = 0;i < n;i++) {
            for(int j = 0;j < m;j++) {
                if(a[i][j] == 'N' || a[i][j] == 'L' || a[i][j] == 'O' || a[i][j] == 'S') {
                    dir = a[i][j];
                    x = i; y = j;
                    i = n;
                    break;
        ...
        for(int i = 0;i < sn;i++) {
            // cout << x << " " << y << nl;
            if(ins[i] == 'F') {
                pair<int, int> coords = move(x, y, dir);
                if(coords.first == -1 || coords.first == n || coords.second == -1 || coords.second == m) continue;
                if(a[coords.first][coords.second] == '#') {
                    continue;
                }
