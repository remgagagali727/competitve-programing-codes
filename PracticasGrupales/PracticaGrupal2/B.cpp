...
vector<char> direct = {'N', 'L', 'S', 'O'};

char rotate(char dir, char to) {
    int i;
    for(i = 0;i < 4;i++) {
        if(direct[i] == dir) {
            break;
...
// void sol(int t) {
    ...
    while(cin >> t) {
        ...
        for(int i = 0;i < 80;i++) {
            ...
            ld ans2 = LONG_MAX;
            for(int i = 0;i < n;i++) {
                ans1 = min(ans1, m1 / a[i].first + (t - m1) / a[i].second);
                ans2 = min(ans2, m2 / a[i].first + (t - m2) / a[i].second);
            }
            ans1 -= m1 / me.first + (t - m1) / me.second;
            ans2 -= m2 / me.first + (t - m2) / me.second;
            if(ans1 < ans2) {
                l = m1;
            } else {
                r = m2;
        ...
        ld ans = LONG_MAX;
        for(int i = 0;i < n;i++) {
            ans = min(ans, l / a[i].first + (t - l) / a[i].second);
        }
        ans -= l / me.first + (t - l) / me.second;
        int seconds = ans * 3600;
        if(seconds < 0) {
            cout << "The cheater cannot win." << nl;
        } else {
