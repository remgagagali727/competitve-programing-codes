...
bool is2div(string num) {
    int i = num[num.size() - 1] - '0';
    return !(i & 1);
}

bool is3div(string num) {
    ll i = 0;
    for(auto x : num) {
        i += x - 48;
    }
...
bool is4div(string num) {
    ll valor = 0;
    if(num.size() >= 2) {
        valor += num[num.size() - 2] - 48;
        valor *= (ll)10;
    }
    if(num.size() >= 1) {
        valor += num[num.size() - 1] - 48;
    }
    return (valor % 4) == 0;
}

bool is5div(string num) {
