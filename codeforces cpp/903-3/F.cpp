...
void sol() {
    ...
    ll node1 = g.getFurthestMarked(1, 0).second;
    ll node2 = g.getFurthestMarked(node1, 0).second;
    g.setMaxDistances(node1, 0, 0);
    g.setMaxDistances(node2, 0, 0);
    ll ans = LONG_LONG_MAX;
    for(int i = 1;i <= n;i++) {
        ans = min(ans, g.getDistanceOf(i));
    }
    cout << ans << nl;
}
