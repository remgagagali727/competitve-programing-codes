...
void sol() {
    ...
    ull distancia = (begin.first - end.first) * (begin.first - end.first) + (begin.second - end.second) * (begin.second - end.second);
    for(int i = 0;i < n;i++) {
        ull tDistancia = (points[i].first - end.first) * (points[i].first - end.first) + (points[i].second - end.second) * (points[i].second - end.second);
        if(tDistancia <= distancia) {
            posible = false;
        }
    }
    if(posible) {
