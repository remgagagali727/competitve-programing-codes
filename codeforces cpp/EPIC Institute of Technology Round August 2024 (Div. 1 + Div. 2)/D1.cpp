...
void sol() {
    ...
    map<ll,ll> value;
    pot2[0] = 1;
    for(int i = 1;i <= k;i++) pot2[i] = 2 * pot2[i - 1];
    for(int i = 1;i < n;i++) {
        int h; cin >> h;
    }
    coords[0] = 0;
    ...
    for(int i = 1;i <= n;i++) {
        ...
        l2 = getLog2(i);
        dis = pot2[k - l2];
        posible += check(coords[padre], coords[i], dis);
        // if(c1 < n) {
        //     l2 = getLog2(c1);
        //     dis = pot2[k - l2];
        //     posible += check(coords[i], coords[c1], dis);
        //     posible += check(coords[i], coords[c2], dis);
        // }
    }
    while(q--) {
        int x, y;
