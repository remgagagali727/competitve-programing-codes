...
void sol() {
    ...
    cin >> n >> m;
    queue<int> q;
    vector<bool> posible(n + 1, true);
    vector<int> criticPoints(n + 1, INT_MAX);
    vector<pair<int,int>> salto;
    sort(all(salto));
    for(int i = 1;i < n;i++) {
        salto.pb({i, i + 1});
    }
    ...
    for(int i = 1;i <= n;i++) criticPoints[i] = i;
    for(auto ps : salto) {
        criticPoints[ps.second] = min(criticPoints[ps.second], criticPoints[ps.first] + 1);
    }
    for(auto ps : salto) {
        ll eliminar = ps.second - criticPoints[ps.second];
        for(int i = ps.first + 1;i <= eliminar;i++) {
            posible[i] = false;
        }
    }
    for(int i = 1;i < n;i++) cout << posible[i];
    cout << nl;
}
