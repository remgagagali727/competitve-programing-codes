...
void sol() {
    ...
    cin >> n >> k;
    int ans = 0;
    int maxVal = expBin(2, k);
    Trie trie(k);
    vector<int> vc(n);
    bool dupped = false;
    pair<int, int> pairBest;
    for(int i = 0;i < n;i++) {
        cin >> vc[i];
        int a = trie.insert(vc[i]);
        if(a == ALREADY_EXIST) {
            dupped = true;
            pairBest = {vc[i], vc[i]};
        }
    ...
    if(!dupped) {
        pairBest = trie.getMinXor();
    }

    ans = (maxVal - 1) ^ (pairBest.first & pairBest.second);

    int cnt = 0;
    for(int i = 0;i < n;i++) {
        if(cnt == 2) break;
        if(pairBest.first == vc[i] || pairBest.second == vc[i]) {
            cout << i + 1 << " ";
            cnt++;
        }
    }
