...
void sol() {
    ...
    i = j = pos - 1;
    bool direccion = DER;
    ll currHealth = valores[j];
    ll acumDerecha = 0;
    ll acumIzquiera = 0;
    vector<bool> tomado(n);
    i--, j++;
    while(i >= 0 && j < n) {
        if(-acumDerecha > currHealth && -acumIzquiera > currHealth) {
            cout << "NO" << nl;
            break;
        }
        if(direccion == DER) {
            if(tomado[j] == false) {
                acumDerecha += valores[j];
                tomado[j] = true;
            }
            if(acumDerecha > 0) {
                currHealth += acumDerecha;
                acumDerecha = 0;
            }
            if(-acumDerecha > currHealth) {
                direccion = IZQ;
                continue;
            }
            j++;
            ...
            if(tomado[i] == false) {
                acumIzquiera += valores[i];
                tomado[i] = true;
            }
            if(acumIzquiera > 0) {
                currHealth += acumIzquiera;
                acumIzquiera = 0;
            }
            if(-acumIzquiera > currHealth) {
                direccion = DER;
                continue;
            }
            i--;
