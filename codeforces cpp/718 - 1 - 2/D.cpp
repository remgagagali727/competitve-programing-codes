...
// void sol(int t) {
    ...
    if(k & 1) {
        ...
        vector<vector<ull>> cdp(n, vector<ull>(m));
        while(k--) {
            for(int i = 0;i < n;i++) {
                for(int j = 0;j < m;j++) {
                    ull ans = LONG_LONG_MAX;
                    if(i != 0)
                        ans = min(ans, dp[i - 1][j] + hor[i - 1][j]);
                    if(i != n - 1) 
                        ans = min(ans, dp[i + 1][j] + hor[i][j]);
                    if(j != 0)
                        ans = min(ans, dp[i][j - 1] + ver[i][j - 1]);
                    if(j != m - 1)
                        ans = min(ans, dp[i][j + 1] + ver[i][j]);
                    cdp[i][j] = ans;
                }
            }
            dp = cdp;
