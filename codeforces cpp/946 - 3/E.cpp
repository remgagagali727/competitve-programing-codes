...
void sol() {
    int n, x;
    cin >> n >> x;
    vector<pair<int, int>> meses(n);
    int maxHappines = 2;
    for(int i = 0;i < n;i++) {
        cin >> meses[i].fi >> meses[i].se;
        maxHappines += meses[i].se;
    }
    vector<vector<ull>> dp(n, vector<ull>(maxHappines));
    
    for(int i = 0;i < n;i++) {
        ull curr = i * x;
        for(int j = 0;j < maxHappines;j++) {
            if(i == 0) {
                if(j == 0 || (meses[i].first == 0 && j == meses[i].second)) {
                    dp[i][j] = 0;
                } else {
                    dp[i][j] = ullinf;
                }
                ...
                ull ans = dp[i - 1][j];
                if(j - meses[i].second >= 0)
                    if(curr - dp[i - 1][j - meses[i].second] >= meses[i].first) ans = min(ans, dp[i - 1][j - meses[i].second] + meses[i].first);
                dp[i][j] = ans;
            }
        }
    }
    ull ans = 0;
    for(int i = 0;i < maxHappines;i++) {
        if(dp[n - 1][i] != ullinf) {
            ans = i;
        }
    }
