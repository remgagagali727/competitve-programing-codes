...
struct SegmentTree{
    ...
    stNode unir(stNode n1, stNode n2) {
        stNode toReturn;
        toReturn.v = max(n1.v, n2.v);
        return toReturn;
    }

    stNode inicializar(ll v) {
        stNode toReturn;
        toReturn.v = v;
        return toReturn;
    }
...
void sol() {
    ...
    for(int i = 0;i < n;i++) {
        ...
        hr.second = max(hr.second, ns[i] - 1);
        if(i == hr.second) {
            rangos[hr.second + 1] = 2;
            hr = {-1, -1};
        }
    }
    string s;
    cin >> s;
    int rangosQueRompen = 0;
    char last = 'R';
    for(int i = 0;i < n;i++) {
        if(last == 'L' && s[i] == 'R') {
            if(rangos[i] != 2) rangosQueRompen++;
        }
        last = s[i];
    }
    while(q--) {
        ...
        if(s[k] == 'L') {
            s[k] = 'R';
            if(s[k - 1] == 'L' && s[k + 1] == 'L') {
                if(rangos[k] != 2) rangosQueRompen++;
            } else if(s[k - 1] == 'L' && s[k + 1] == 'R') {
                if(rangos[k] != 2) rangosQueRompen++;
                if(rangos[k + 1] != 2) rangosQueRompen--;
            } else if(s[k - 1] == 'R' && s[k + 1] == 'L') {
        ...
        cout << (rangosQueRompen > 0 ? "no" : "yes") << nl; 
    }
}
