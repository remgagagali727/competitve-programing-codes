...
int shape(string& a, string& b, int i, int type) {
    if(type == UL) {
        int c = a[i] + a[i + 1] + b[i];
        return c > 1;
    } else if(type == DL) {
        ...
        int c = a[i + 2] + a[i + 1] + b[i + 2];
        return c > 1;
    } else {
        int c = a[i + 2] + b[i + 1] + b[i + 2];
        return c > 1;
    }
    return 0;
}
...
void sol() {
    ...
    vector<vector<int>> dp(n / 3 + 2, vector<int>(3));
    dp[0][0] = 0;
    int j = 0;
    // 1 = UpperLeft and DownRight, 2 = DownLeft and UpperRight
    for(int i = 0;i < n;i+=3, j++) {
        dp[j][1] = max(dp[j][1], dp[j][0] + shape(a, b, i, UL));
        dp[j][2] = max(dp[j][2], dp[j][0] + shape(a, b, i, DL));
        dp[j + 1][0] = max(dp[j][0] + line(i, a) + line(i, b), dp[j + 1][0]);
        dp[j + 1][0] = max(dp[j][1] + shape(a, b, i, DR), dp[j + 1][0]);
        dp[j + 1][0] = max(dp[j][2] + shape(a, b, i, UR), dp[j + 1][0]);
        dp[j + 1][1] = max(dp[j + 1][1], dp[j][1] + line(i + 2, a) + line(i + 1, b));
        dp[j + 1][2] = max(dp[j + 1][2], dp[j][2] + line(i + 1, a) + line(i + 2, b));
    }
    cout << dp[j][0] << nl;
}
