...
void sol() {
    ...
    vector<int> breaks;
    for(int i = 1;i < n;i++) {
        bool megaBool = qry(i, i + 1);
        if(!megaBool) breaks.pb(i);
    }
    breaks.pb(n);
    int l = 0, r = breaks.size() - 1;
    while(l + 1 < r) {
        int m = (l + r) / 2;
        int dif = (m - 0) % 2;
        bool qb = qry(1, breaks[m]);
        if(qb ^ dif) {
            l = m;
        } else {
            r = m;
    ...
    bool q1 = qry(breaks[l], breaks[r]);
    bool q2 = qry(breaks[r], breaks[l]);
    if(q1 != q2) cout << "! " << breaks[l] << nl;
    q1 = qry(breaks[0], breaks[r]);
    q2 = qry(breaks[r], breaks[0]);
    if(q1 != q2) cout << "! " << breaks[0] << nl;
    else {
        int constl = breaks[r];
        l = breaks[l] + 1, r = breaks[r];
        while(l + 1 < r) {
            int m = (l + r) / 2;
            bool qb = qry(constl, m);
            if(qb) r = m;
            else l = m;
        }
        cout << "! " << l << nl;
