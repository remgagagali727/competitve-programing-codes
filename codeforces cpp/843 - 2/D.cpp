...
void sol() {
    ...
    for(int i = 0;i < n;i++) {
        cin >> a[i];
        indices[a[i]] = i + 1;
        maxNode = max(maxNode, a[i]);
    }
    int s, t;
    cin >> s >> t;
    vector<int> divide(maxNode + 1);
    divide[1] = 1;
    ...
    int maxPrimo = 0;
    if(!primos.empty()) maxPrimo = primos[primos.size() - 1];
    Graph g(maxPrimo + maxNode + 5, true);
    for(int i = 0;i < n;i++) {
        int v = a[i];
        while(v > 1) {
            int div = divide[v];
            g.add(a[i], maxNode + div);
            while(v % div == 0) {
                v = v / div;
                if (v == 1) break;
            }
    ...
    indices[sV] = s; indices[tV] = t;
    g.bfs(tV);   
    if(g.getDistance(sV) == -1) {
        cout << -1 << nl;
        return;
    }
    string ans = "";
    ...
    while(true) {
        if(sV <= maxNode) {
            tiempo++;
            ans.append(to_string(indices[sV])).append(" ");
        }
        if(sV == tV) break;
        sV = g.getParent(sV);
    }
    cout << tiempo << nl << ans << nl;
}
