...
class Graph{
    ...
    ll dfs(ll v, int color) {
        if(marked[v] != 0) return 0;
        // cout << v << " " << color <<  nl;
        ll ans = color;
        marked[v] = color;
        color = color==1?-1:1;
...
void sol() {
    ...
    for(int i = 0;i < n;i++) {
        ...
        for(int j = 0;j < i;j++) {
            auto cj = circles[j];
            ll a = dist(ci[0], ci[1], cj[0], cj[1]);
            if(a == pow((ci[2] + cj[2]), 2)) {
                g.add(i, j);
            }
            //  else if(a < pow((ci[2] + cj[2] + 1), 2)) {
            //     g.add(i, j);
            // }
        }
    }
    ...
    for(int i = 0;i < n;i++) {
        ...
        // cout << nl;
        // cout << valor << nl;
        if(valor != 0 && valor != INVALID) {
            posible = true;
        }
    }
    cout << (posible ? "YES" : "NO") << nl;
}
